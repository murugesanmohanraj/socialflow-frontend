...
import { connectInstagramAccount } from "../../services/accountsApi";

function InstagramConnectView() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await connectInstagramAccount(email.trim(), password);
      navigate("/accounts");
    } catch (requestError) {
      setError(
  ...
  return renderPage(
    <div className="mx-auto w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
      ...
      <div className="text-center">
        <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[#fce7f3] text-2xl font-bold text-[#be185d]">
          ◎
        </span>
        ...
        <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-slate-500">
          Enter the Instagram email and password for the account you want to use
      ...
      <form className="mt-7 space-y-4" onSubmit={handleSubmit}>
        ...
        <button
          ...
          className="w-full rounded-xl bg-[#be185d] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#9d174d] disabled:cursor-not-allowed disabled:bg-slate-400"
        >
...
export default InstagramConnectView;
