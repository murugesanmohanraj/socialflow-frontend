...
import { connectFacebookAccount } from "../../services/accountsApi";

function FacebookConnectView() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await connectFacebookAccount(email.trim(), password);
      navigate("/accounts");
    } catch (requestError) {
      setError(
        requestError instanceof Error
          ? requestError.message
          : "Unable to connect Facebook account.",
      );
    } finally {
  ...
  return renderPage(
    <div className="mx-auto w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
      ...
      <div className="text-center">
        <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[#e8f0fe] text-2xl font-bold text-[#1d4ed8]">
          f
        </span>
        ...
        <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-slate-500">
          Enter the Facebook email and password for the account you want to use
      ...
      <form className="mt-7 space-y-4" onSubmit={handleSubmit}>
        ...
        <button
          ...
          className="w-full rounded-xl bg-[#1976d2] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#145bb0] disabled:cursor-not-allowed disabled:bg-slate-400"
        >
          {isSubmitting ? "Connecting..." : "Connect Facebook"}
        </button>
      </form>
    </div>,
  );
...
export default FacebookConnectView;
